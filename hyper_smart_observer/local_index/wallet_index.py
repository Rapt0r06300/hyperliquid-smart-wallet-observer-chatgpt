...
class WalletLocalIndex:
    """In-memory fallback index for fast local scans.

    This is deliberately local-only: it accepts already imported/cache rows and
    never performs network reads.
    """
...
def fake_wallet(index: int) -> IndexedWallet:
    ...
    return IndexedWallet(
        ...
        sources={"local_benchmark_fixture"},
    )
